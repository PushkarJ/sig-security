...
// deliberately permissive (any other arguments are silently ignored) to
// match the Python original's `"--check" in sys.argv[1:]`.
...
func run(args []string) error {
	...
	result, err := buildStatements(filesGlob, overrides)
	if err != nil {
		return err
	}
	...
	if check {
		if statementsChanged || metadataChanged {
			return fmt.Errorf("feed is STALE — run: %s", invocationHint)
		}
	...
	for _, s := range result.statements {
		cveSet[s.Vulnerability.Name] = struct{}{}
	}
	what := "metadata only"
	if statementsChanged {
